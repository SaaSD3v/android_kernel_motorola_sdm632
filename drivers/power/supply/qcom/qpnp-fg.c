			...
			if (fg_est_dump)
				schedule_work(&chip->dump_sram);
				   ...
				   data = of_get_property(profile_node,
					...
					memcpy(chip->thermal_coefficients,
						data, len);
					rc = fg_mem_write(chip,
						chip->thermal_coefficients,
							THERMAL_COEFF_ADDR,
							THERMAL_COEFF_N_BYTES,
							THERMAL_COEFF_OFFSET,
							0);
					if (rc)
